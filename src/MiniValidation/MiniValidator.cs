        ...
        int currentDepth = 0)
    {
        ...
        if (recurse && currentDepth <= MaxDepth)
        {
            ...
            if (target is IEnumerable)
            {
                ...
                isValid = TryValidateEnumerable(target, recurse, workingErrors, validatedObjects, validationResults, prefix, currentDepth) && isValid;
            }
                ...
                foreach (var property in propertiesToRecurse)
                {
                    ...
                    if (propertyValue != null)
                    {
                        ...
                        if (propertyDetails.IsEnumerable)
                        {
                            ...
                            isValid = TryValidateEnumerable(propertyValue, recurse, workingErrors, validatedObjects, validationResults, thePrefix, currentDepth) && isValid;
                        }
                        else
                        {
                            var thePrefix = $"{prefix}{propertyDetails.Name}."; // <-- Note trailing '.' here
                            isValid = TryValidateImpl(propertyValue, recurse, workingErrors, validatedObjects, validationResults, thePrefix, currentDepth + 1) && isValid;
                        }
                    }
                }
            }
